...
export default function LanguageDropdown({ text }) {
  ...
  return (
    <a
      ...
      }  px-3 py-4 lg:py-2 cursor-pointer`}
    >
      <i>
        <img
          className={`inline-block ${lang === "english" ? "mr-2" : "ml-2"}`}
          src={
            lang === "farsi"
              ? process.env.imagesPrefix + "/img/flags/USA.png"
              : process.env.imagesPrefix + "/img/flags/Iran.png"
          }
        />
      </i>
      <span>{lang === "english" ? "فارسی" : "English"}</span>
    </a>
  );
}
